...
Sky::Sky(Microsoft::WRL::ComPtr<ID3D11SamplerState> samplerState, std::shared_ptr<Mesh> mesh, std::shared_ptr<SimpleVertexShader> vertexShader, std::shared_ptr<SimplePixelShader> pixelShader) :
...
{ 
	...
	Graphics::Device->CreateRasterizerState(&rasterDesc, rasterizer.GetAddressOf());

	D3D11_DEPTH_STENCIL_DESC dbDesc = {};
	dbDesc.DepthEnable = true;
	dbDesc.DepthFunc = D3D11_COMPARISON_LESS_EQUAL;
	Graphics::Device->CreateDepthStencilState(&dbDesc, depthBuffer.GetAddressOf());
}
...
// --------------------------------------------------------
void Sky::CreateCubemap(const wchar_t* right,
	const wchar_t* left,
	const wchar_t* up,
	const wchar_t* down,
	const wchar_t* front,
	const wchar_t* back)
{
	...
	Graphics::Device->CreateShaderResourceView(cubeMapTexture.Get(), &srvDesc, shaderResourceView.GetAddressOf());
}
...
void Sky::Draw(std::shared_ptr<Camera> camera)
{
	...
	pixelShader->SetShaderResourceView("SkyBox", shaderResourceView);
